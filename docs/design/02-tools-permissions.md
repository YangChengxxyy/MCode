...
内建 `write`、`edit` 与 `exec` 持有同一 process-wide execution lease,串行化同进程写者与 identity-bound 启动;`edit` 的规划、规划失败与 no-op 路径不获取 lease,只有形成真实 changed payload 后才在 publication 前获取。`write`/`edit` 把 lease 移入 supervised publication worker,丢掉外层 future 也不会在仍可能 publish 时释放。不把跨进程 advisory lock 或 per-file 队列夸大为安全边界。生命周期错误不得回显 `write.content` 或其他正文。

## 6. 内建工具清单(M1)

read / write / edit / bash / exec / grep / find —— 全部实现 `Tool`,进同一个 Registry,作为插件 API 的 reference 实现。
...
- [ ] `concurrency: Exclusive` 工具(`bash`/`exec`)与 parallel 工具的混合调度策略
