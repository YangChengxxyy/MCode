...
| --- | --- | --- |
| Manager Plugin `mcode:plugin@0.0.1` / `mcode:plugin/manager@0.0.1`（1 world） | 唯一 import 是 `mcode:plugin/feature-service@0.0.1` 的 typed Pack selection/activation 与 task transport；exports 是 `mcode:plugin/manager-lifecycle@0.0.1` lifecycle 和 Core-facing `mcode:plugin/manager-tasks@0.0.1` string ingress | Host 先绑定 canonical family、Manager ID 与 generation，将 `1..=9007199254740991` 的 generation 放入 typed `initialization-context` 调用 `initialize`，再选择 family-specific typed decode |
...
Manager lifecycle 分成 PREPARING 与 CURRENT 两个 admission phase。`initialize` 与显式一步式 `poll_preparation` 在 PREPARING 运行，FeatureService admission 保持关闭；原子发布后，上层 coordinator 必须从 Director 取得 opaque current-generation view，并以该 view 作为 expected tag 显式调用一次 `poll_current`。Director 在同一 state lock 内重新选择 exact family/record/generation、校验 expected tag 并取得 activity，释放锁后才等待 owner 与执行 guest poll；stale tag 在 guest 前拒绝，调用不循环、不自动 timer poll，也绝不改投 replacement。tag 中的 revision 只是 selection observation，不参与 generation identity；仅 revision 前进而 exact generation 未变时旧 tag 仍有效，返回值或 post-selection error 改以实际 selection revision 盖章。CURRENT poll 是唯一允许 Manager 使用 FeatureService imports 的 lifecycle call。FeatureService 的 `configured-packs` 在 generation fence 内返回 Host-issued `psel1` stamp 与 exact configured ordered executable Pack IDs，最多 256 项；root composition revision 与 Manager registry revision 独立，Providers/Usage 保留顺序，UI 只包含 executable runtime、排除 declarative themes。配置前进会为同一 generation 旋转 stamp；PREPARING/retired generation fail closed。`activate-packs` 只接受本 generation 当前 exact selection stamp，按顺序完成 canonical installation/digest/world 校验与 typed Store instantiation；全部候选成功并在最终 root/generation revalidation 后才一次替换 active set，empty 使用同一路径 deactivate-all，任一失败保留旧 set，同 stamp 重试幂等。`Ready|Pending` 保持 current；`Stopping|Stopped`、lifecycle rejection、trap、fuel/deadline 或 Store failure只 compare-remove exact generation，再关闭 admission、cancel/drain、尝试一次 bounded shutdown并无条件 dispose，authority revision/target/high-water 不回退。每次 initialize、preparation poll、current poll 与 retirement shutdown 使用独立的 bounded owner-bound fuel lease，不能由长寿 generation 的旧 remainder 挤压后续 cleanup。shutdown guest outcome仅供 test observation；生产清理把它视为 best effort，结果不重新开放 admission、也不阻止 dispose。

Manager、FeaturePack 与 ProviderPack 的 sole-current ABI artifacts 已落地；FeaturePack 与 ProviderPack 已具备 parser-checked source/current goldens 和逐 world semantic JSONL goldens。三者的目标字段表、exact signatures、artifact slice 边界和 T7 gates 分别由本文、[07-pack-abi.md](07-pack-abi.md) 与 [08-provider-pack-abi.md](08-provider-pack-abi.md) 定义。这不表示 T7 已完成：artifact validators 仍缺 closed `AdapterContractV1`、static dummy context counter、完整 pure decoder reducers、13 candidate worlds × 13 validators rejection matrix 与完整 binary static preflight，完成后方可进入 T8。11 个 Feature worlds 共享唯一 package version `mcode:feature-pack@0.0.1`，但每个 world 的 binding、type namespace 与 current-only golden 物理独立；全部 MCode-owned FeaturePack interface 同为 `@0.0.1`，且不保留 `abi_v1.json`、historical golden、compatibility parser/adapter、ABI alias、dual-read 或 fallback。Manager task wire 的公开 `abiVersion` 固定为精确 string `"0.0.1"`，wire 仅有 `kind=featureService` 且不接受 caller-supplied family；Manager 只能使用 Host 在 `initialize` typed context 中提供的 active generation 构造 task wire。`operationId` 是 `1..=128` bytes 的 declarative canonical key，与 Host vault operation authority 共用唯一 validator；Host 在 body decode、Pack 与 transport 前，以最多 128 项的已绑定声明集 fail closed。`taskId` 仍是 Host-issued task instance identity。`start-task` 在 allocation 前，以及 `poll-task`/`cancel-task` 的 control envelope 无法严格解码完整 `operationId/taskId/generation` identity 时，统一返回仅含 `abiVersion/kind/state/error` 的 `FeatureTaskRejection`；一旦完整 control identity 解码成功，包括 lookup 返回 `UnknownTask`，即使用带完整 identity 的 `FeatureTaskError` assigned shape。JSON 只承载固定字段、有界且 family-body typed 的 task envelope，公共 API 不接受或返回 `serde_json::Value`、raw opaque body、无界 map 或延迟解释字段；不创建共享 `PackOperation`。Provider `toolChoice` 固定为 `Unset|Auto|None|Specific`，每种 wire 冻结 omitted 语义。
