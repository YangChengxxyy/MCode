...
>
> 这是 MCode 的首个开发者预览 design authority：公开的 MCode product/workspace release、所有第一方 Manager/Pack package/release，以及 MCode-owned Manager、FeaturePack、ProviderPack 的 package/world/interface 均为 sole-current `0.0.1`。不保留历史 artifact、alias、dual-read、fallback 或 coexistence。内部 schema format tag（包括 `formatVersion=1` 与 `AdapterContractV1.version=1`）及外部 dependency/WASI version 不是 MCode release version，保持不变。
...
| [07-pack-abi.md](07-pack-abi.md) | 11 个 FeaturePack world 的 current-only 字段、Host methods、边界与生命周期 authority |
| [08-provider-pack-abi.md](08-provider-pack-abi.md) | ProviderPack zero-import surface、catalog、AdapterContractV1 与 decoder authority |
...
- Providers 为 `N`；Usage 为 canonical source key 唯一的 `N`，每次 operation 由 Host-issued source view/stamp 在 strict decode 后唯一绑定；UI 为一个 product UiPack 加 `N` 个 Theme-role Pack；包括 Web 在内的其余 family 均为 Host-wide `0..1` singleton。
...
目录、权威文件和凭据契约见 [03-plugins.md](03-plugins.md)；Manager ABI 与 runtime 说明见 [05-plugin-impl.md](05-plugin-impl.md)，FeaturePack/ProviderPack 的 T7 authority 见 [07-pack-abi.md](07-pack-abi.md) 与 [08-provider-pack-abi.md](08-provider-pack-abi.md)。
