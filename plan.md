...
- T7 已完成。T8 已落地 runtime/preflight、Manager loading/lifecycle/generation/current/task dispatch、generation-bound bounded Pack candidate loading 与 exact configured Pack atomic activation；T8 仍未整体完成。
...
- [ ] **TODO(T8.3)**：generation-bound `configured-packs`、atomic `activate-packs` 与 bounded current Manager `start/poll/cancel-task` dispatch 已接通，并覆盖 exact tag、revision observation、reload cancel/quiescence、future-drop、trap 与 wire bounds；继续接通 family-specific Pack invoke/pull、Host task table、terminal cancel/stale 与 shutdown/drop 门禁。
...
- Manager task wire 保留 `start-task/poll-task/cancel-task` JSON 网关，但删除 direct `Web/Mcp/AgentRun` task/capability，只增加唯一 `FeatureService` kind；其公开 `abiVersion` 是精确 string `"0.0.1"`。Host 由 caller Manager identity 绑定 strict family，再进入 family-specific typed decoder。`operationId` 是与 vault operation authority 共用唯一无分配 validator 的 `1..=128` bytes declarative canonical key；Host 在 body decode、Pack 与 transport 前，以最多 128 项的已绑定声明集 fail closed，`taskId` 仍是 Host-issued task instance identity。`start-task` 在 allocation 前，以及 `poll-task`/`cancel-task` 的 control envelope 无法严格解码完整 `operationId/taskId/generation` identity 时，rejection 只含 `abiVersion/kind/state/error`；一旦完整 control identity 解码成功，包括 lookup 返回 `UnknownTask`，即使用带完整 identity 的 `FeatureTaskError` assigned shape。未知、跨 family、未声明或 oversized 请求 fail closed；不是万能 JSON 通道。
- Core 只用 Director 产出的 exact current-generation tag 调用 Manager task exports；三个调用各使用 fresh bounded lease，输入/输出均限制为 `64 KiB`。revision-only advance 可继续使用同 generation 的旧 tag并返回当前 revision observation；reload 取消运行中和排队调用，guest trap、超限输出或已进入 guest 的 future-drop 会 retire/dispose exact generation，绝不改投 replacement。
- Manager sole-current world 只 import `feature-service`，并 export typed `manager-lifecycle` 与 Core-facing string `manager-tasks`。FeatureService 的 `configured-packs` 在 current generation fence 内返回 Host-issued `psel1` stamp 和 exact configured ordered executable Pack IDs（最多 256）；`activate-packs` 只接受本 generation 当前 stamp，按顺序 secure load/typed instantiate 全部候选，并在 final root/generation revalidation 后原子替换 single/multi set；empty 使用同一路径 deactivate-all，失败保留旧 set，同 stamp 重试幂等。
...
- **T8（进行中）**：fixed-12 Manager loading、typed lifecycle、authority generation director、原子 publication gate、cancel/drain、cleanup worker、bounded Pack candidate loading、generation-bound configured/atomic activation 与 current Manager task dispatch 已落地；family-specific Pack execution 与端到端门禁仍按第 0 节 TODO 完成。
