...
- [ ] **TODO(T8.3)**：Manager `0.0.1` 已冻结 Core-facing `manager-tasks` ingress 与 Host-issued `psel1` Pack selection/activation ABI，当前 activation import fail closed 为 `unavailable`；继续接通真实 current Pack execution、atomic selection replacement、generation-bound cancel、RAII waiting、quiescence、stale rejection 与 shutdown/drop 端到端门禁。
...
- Manager sole-current world 只 import `feature-service`，并 export typed `manager-lifecycle` 与 Core-facing string `manager-tasks`。FeatureService 的 `configured-packs` 返回 Host-issued `psel1` stamp 和 exact configured ordered executable Pack IDs（最多 256）；`activate-packs` 以该 stamp 原子替换 single/multi selection，empty selection 表示 atomic deactivate-all。该 ABI 仅有 current `0.0.1`；真实 activation 接通前 Host 稳定返回 `unavailable`，不得伪装成功。
