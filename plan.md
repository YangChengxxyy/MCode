...
- T7 已完成。T8 已落地 runtime/preflight、Manager loading/lifecycle/generation/current dispatch、generation-bound bounded Pack candidate loading 与 exact configured Pack atomic activation；T8 仍未整体完成。
...
- [ ] **TODO(T8.3)**：generation-bound `configured-packs` 与 atomic `activate-packs` 已接通 exact ordered secure load/typed instantiation、final root/generation revalidation、empty deactivate-all、失败保留旧 set 与同 stamp 幂等；继续接通真实 current Pack task execution、cancel/quiescence、stale rejection 与 shutdown/drop 门禁。
...
- **T8（进行中）**：fixed-12 Manager loading、typed lifecycle、authority generation director、原子 publication gate、cancel/drain、cleanup worker、bounded Pack candidate loading 与 generation-bound configured selection 已落地；atomic activation、current Pack execution 及端到端门禁仍按第 0 节 TODO 完成。
