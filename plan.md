...
- [ ] **TODO(T8.3)**：generation-bound `configured-packs` 已返回 exact ordered Pack IDs 并随 root revision 旋转 Host-issued `psel1` stamp；继续接通 atomic `activate-packs`、真实 current Pack execution、cancel/quiescence、stale rejection 与 shutdown/drop 门禁。
...
- Manager sole-current world 只 import `feature-service`，并 export typed `manager-lifecycle` 与 Core-facing string `manager-tasks`。FeatureService 的 `configured-packs` 已在 current generation fence 内返回 Host-issued `psel1` stamp 和 exact configured ordered executable Pack IDs（最多 256）；`activate-packs` 将以该 stamp 原子替换 single/multi selection，empty selection 表示 atomic deactivate-all，接通前稳定返回 `unavailable`。
