...
use super::common::client_side_sse::{
    DEFAULT_MAX_SSE_EVENT_SIZE, ExponentialBackoff, SseRetryPolicy, SseStreamContext,
    SseStreamReconnect,
};
use crate::{
    RoleClient,
...
impl<C: StreamableHttpClient> SseStreamReconnect for StreamableHttpClientReconnect<C> {
    ...
    fn bind_stream_context(&mut self, ctx: SseStreamContext) {
        self.ctx = ctx;
    }
    ...
    fn retry_connection(&mut self, last_event_id: Option<&str>) -> Self::Future {
        ...
        let extra = self.ctx.extra_get_delay();
        Box::pin(async move {
